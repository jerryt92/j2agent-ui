        ...
        <div class="md-viewer-shell">
          <div class="md-viewer-panel">
            ...
            <div
              ...
              aria-live="polite"
            >
              <ElIcon class="md-viewer-spinner">
                <Loading />
              </ElIcon>
...
const activateMarkdown = async () => {
  ...
  // 先保留 reønderMarkdownCached 生成的完整正文，图表随后并行挂载。
  // 这样打开文档无需等待 Mermaid/PlantUML/Vega 完成，行为与门户文档一致。
  void renderMarkdownBlocks(root, {
    ...
    console.error('Markdown 预览图表渲染失败:', error)
  })
  // hash 定位不再被图表渲染阻塞；图表完成后再校正一次位置。
  scrollToPendingHash()
}
